...
The system lives in `{{HOME}}\.agents\system\`. This skill only routes you there — start with the file the current situation routes to, and read another routed file only when the task actually reaches that decision (never all of them up front).

| Situation                                                      | Read                                                                                                       |
| -------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| Delegating anything; choosing subagent type/model/tier         | `{{HOME}}\.agents\system\10-DISPATCH.md`, then **only** matching `11-INVENTORY-{cursor\|claude\|codex}.md` |
| Is it done? Retry or escalate? Ask the user? Wrong direction?  | `{{HOME}}\.agents\system\20-JUDGMENT.md`                                                                   |
| Writing the actual dispatch prompt                             | `{{HOME}}\.agents\system\30-TEMPLATES.md`                                                                  |
| Editing the system files themselves                            | `{{HOME}}\.agents\system\40-MAINTENANCE.md`                                                                |
