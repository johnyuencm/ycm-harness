...
if (rootAt < 0 || !args[rootAt + 1] || args.length !== 2) {
  process.stderr.write(`${JSON.stringify({ ok: false, reason_code: "pm_canary_root_required" })}\n`);
  process.exit(2);
}
...
async function installedRuntime() {
  ...
  if (path.basename(scripts) !== "scripts" || path.basename(script) !== "pm-installed-canary.mjs") {
    throw new Error("pm_canary_runtime_missing");
  }
  for (const [candidate, kind] of [
    ...
    if (await realpath(candidate).catch(() => undefined) !== candidate) throw new Error("pm_canary_runtime_missing");
  }
  const relative = path.relative(await realpath(pluginRoot), await realpath(runtime));
  if (!relative || relative.startsWith("..") || path.isAbsolute(relative)) throw new Error("pm_canary_runtime_missing");
  return runtime;
}

try {
  const runtime = await installedRuntime();
  const { runPmInstalledManualCanaryTrace } = await import(pathToFileURL(runtime).href);
  const trace = await runPmInstalledManualCanaryTrace(path.resolve(args[rootAt + 1]));
