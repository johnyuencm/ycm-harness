...
name: create-skill
description: >-
  Create Cursor Agent Skills. Prefer abstract, portable workflows that adapt to
  any repo; put project-specific contracts only in project-scoped skills or
  examples. Use when authoring a new skill or asking about SKILL.md structure.
---
...
### 1. Abstract by default (portable skills)

Default to the **most abstract** skill that still works. Skills often travel across repos, products, and teams. Project-specific names, paths, commit SHAs, and one-off contracts lock the skill to a single context and go stale.

| Prefer | Avoid (unless the skill is explicitly project-scoped) |
|--------|--------------------------------------------------------|
| Discover contracts from *this* repo’s docs, ADRs, lessons, intentional commits | Hard-coding one product’s dependency list or install layout |
| “Who owns this dependency: repo vs environment vs marketplace?” | Naming a specific third-party skill pack as the only allowed source |
| Generic corruption checks (docs vs install, reintroduced deleted trees) | Copy-pasting one incident’s file paths as eternal rules |
| “Adapt paths to the repository” | Absolute home paths, org/repo URLs, internal ticket IDs |

**When concrete examples help:** put them in `examples.md` or a clearly labeled optional appendix — not in the mandatory workflow. The agent should learn the *pattern*, then bind it to the current repo.

**Exception:** A skill stored only under a single project’s `.cursor/skills/` and never reused elsewhere may name that project’s tools. If the skill is personal (`~/.cursor/skills/`), bundled in a multi-repo plugin, or otherwise portable — stay abstract.
...
### 6. Project-Specific Portable Skills
- ❌ Hard-coding one repo’s package names, vendor paths, internal service names, or incident filenames into a skill meant for many repos
- ❌ Turning a one-off postmortem into mandatory workflow steps that only apply there
- ✅ Teach the pattern (“discover ownership from docs + intentional commits”); bind examples in `examples.md` or an optional appendix
- ✅ Only ship repo-named contracts in a **project** skill under that repo’s `.cursor/skills/`
...
3. Decide portability: personal/plugin/multi-repo → **abstract by default**; single-project only → concrete names allowed
4. Outline the main sections needed
5. Identify if supporting files or scripts are needed
6. If the draft came from one incident, plan a strip pass (remove proper nouns and one-repo paths)
